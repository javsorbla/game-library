...
class ApiEndpointTests(TestCase):
    ...
    def test_rating_created_when_marking_played(self):
        game = Game.objects.create(name="New Game")
        mark_url = reverse('game_mark_played', args=[game.id])
        res = self.client.post(mark_url)
        self.assertEqual(res.status_code, 200)
        # rating object should exist with default value
        from .models import GameRating
        rating = GameRating.objects.filter(user=self.user, game=game).first()
        self.assertIsNotNone(rating)
        self.assertEqual(rating.rating, 1000)
        # serializer should include tier_rating; use a fake request with user
        from rest_framework.test import APIRequestFactory
        factory = APIRequestFactory()
        fake_req = factory.get('/')
        fake_req.user = self.user
        rdata = GameSerializer(game, context={'request': fake_req}).data
        self.assertEqual(rdata.get('tier_rating'), 1000)

    def test_tier_pair_and_submit_and_rankings(self):
        # create a couple of played games and walk through the flow
        g1 = Game.objects.create(name="G1")
        g2 = Game.objects.create(name="G2")
        # mark them played which also creates ratings
        ...
        self.assertEqual(res.status_code, 200)
        # verify response is list of two games
        ...
        payload = {'winner': g1.id, 'loser': g2.id}
        res2 = self.client.post(submit_url, payload, format='json')
        self.assertEqual(res2.status_code, 200)
        # winner rating should now be > 1000 and loser < 1000
        from .models import GameRating
        r1 = GameRating.objects.get(user=self.user, game=g1)
        r2 = GameRating.objects.get(user=self.user, game=g2)
        self.assertGreater(r1.rating, 1000)
        self.assertLess(r2.rating, 1000)

        # rankings endpoint should order by rating descending
        rank_url = reverse('tier_rankings')
        res3 = self.client.get(rank_url)
